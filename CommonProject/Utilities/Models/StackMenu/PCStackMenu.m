...
- (void)show:(PCStackMenuBlock)block
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
		for(int i = 0; i < [_items count]; i++)
		{
			...
			r.origin.y += i * r.size.height * (_menuDirection == PCStackMenuDirectionClockWiseUp || _menuDirection == PCStackMenuDirectionCounterClockWiseUp ? -1.2 : 1);
			stackItem.frame = r;
			CGPoint point = [self centerPointForRotation:stackItem];
			stackItem.transform = CGAffineTransformMakeRotationAt(i * (_menuDirection == PCStackMenuDirectionClockWiseUp || _menuDirection == PCStackMenuDirectionClockWiseDown ? 0 : -2) * M_PI / 180, point);
			stackItem.alpha = 1.0;
		}
				  ...
				  menuDirection:(PCStackMenuDirection)direction
{
	...
	for(int i = 0; i < max; i++)
	{
		CGRect r = CGRectMake((direction == PCStackMenuDirectionClockWiseUp || direction == PCStackMenuDirectionCounterClockWiseDown) ? 0 : startPoint.x,
							  startPoint.y + (direction == PCStackMenuDirectionClockWiseDown || direction == PCStackMenuDirectionCounterClockWiseDown ? 0 : -itemHeight-1),
		...
		PCStackMenuItem *stackItem = [[PCStackMenuItem alloc] initWithFrame:r withTitle:title withImage:image alignment:(direction == PCStackMenuDirectionClockWiseUp || direction == PCStackMenuDirectionCounterClockWiseDown) ? TextAlignementRight : TextAlignementLeft];
