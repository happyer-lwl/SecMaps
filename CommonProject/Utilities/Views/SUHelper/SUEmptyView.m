...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _refreshButton = [MSBase createButton:CGRectMake(0.5f*(frame.size.width-110.0), _desLabel.frame.origin.y+_desLabel.frame.size.height + 20.f, 110.0f, 31.0f) type:UIButtonTypeCustom title:@"重新加载"];
        [_refreshButton setTitleColor:[Config defaultConfig].themeColor forState:UIControlStateNormal];
        _refreshButton.layer.cornerRadius = 5.0f;
        _refreshButton.layer.borderColor = [Config defaultConfig].themeColor.CGColor;
...
-(void) setNetwork:(BOOL) isConnect
{
    if (isConnect) {
        ...
        if (_netWorkView == nil) {
            ...
            UILabel *txtl = [MSBase createLabel:CGRectMake((self.frame.size.width-labelSize.width)/2.f,(_netWorkView.frame.size.height-25.f)/2, labelSize.width, 25.f) font:[UIFont systemFontOfSize:13.f] text:netTile defaultSizeTxt:nil color:ColorFromRGB(58.f, 58.f, 57) backgroundColor:[UIColor clearColor]];
